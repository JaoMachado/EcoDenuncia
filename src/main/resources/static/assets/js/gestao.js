...
function renderTabela(filtro = {}) {
  ...
  const filtroStatusNormalizado = filtro.status
    ? normalizarStatus(filtro.status)
    : "";

  const filtradas = denuncias.filter((d) => {
    if (
      filtroStatusNormalizado &&
      normalizarStatus(d.status) !== filtroStatusNormalizado
    )
      return false;
    if (filtro.data && !d.data.startsWith(filtro.data)) return false;
    if (
      filtro.local &&
      !d.endereco.toLowerCase().includes(filtro.local.toLowerCase())
    )
      return false;
    return true;
  });
  ...
  filtradas.forEach((d) => {
    ...
    const podeConcluir =
      ...
      statusNormalizado === "EM TRATAMENTO";
    ...
    tr.innerHTML = `
            ...
            <td>
                ${
                  podeAssumir
                    ? `<button class="btn btn-sm btn-success" onclick="assumirDenuncia('${d.id}')">Assumir Denúncia</button>`
                    : ""
                }
                ${
                  podeConcluir
                    ? `<button class="btn btn-sm btn-primary" onclick="concluirDenuncia('${d.id}')">Concluir</button>`
                    : ""
                }
            </td>
        `;
    ...
    if (vaiAssumir) {
      try {
        ...
        if (response.ok) {
          ...
          swal(
            "Erro",
            erroMsg || "Não foi possível assumir a denúncia.",
            "error"
          );
        }
      } catch (error) {
    ...
    if (vaiConcluir) {
      try {
        ...
        if (response.ok) {
          ...
          swal(
            "Erro",
            erroMsg || "Não foi possível concluir a denúncia.",
            "error"
          );
        }
      } catch (error) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        swal(
          "Acesso Negado",
          "Você não tem permissão de Gestor ou Admin para ver esta página.",
          "error"
        );
      }
      throw new Error(
        "Falha ao carregar denúncias. Status: " + response.status
      );
    }
